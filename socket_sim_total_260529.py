...
# Latch / HeatSink 통합 소켓 접촉 시뮬레이터 (Total Version - SpinBox 직접 입력 추가 버전)
# 숫자의 단위는 0.01 단위로 변경 가능하도록 스냅 처리를 0.01 단위로 변경했습니다.
# 입력 파라미터를 슬라이더와 동시에 직접 숫자로 입력하여 연동할 수 있도록 Spinbox를 추가하였습니다.
...
class SocketSimulationTotalApp:
    def __init__(self, root):
        ...
        self.latch_vars = {
            ...
            "dial_val": tk.DoubleVar(value=10.0),
            "dial_offset": tk.DoubleVar(value=0.4)
        }
        self.latch_spinboxes = {}
        self.latch_sliders = {}
        self.create_slider(self.latch_input_group, "Rubber 최대 높이 (S_max)", self.latch_vars, self.latch_sliders, self.latch_spinboxes, "s_max", 0.1, 1.5)
        self.create_slider(self.latch_input_group, "Rubber Hard Stop (S_hard)", self.latch_vars, self.latch_sliders, self.latch_spinboxes, "s_hard", 0.2, 1.3)
        self.create_slider(self.latch_input_group, "MPGA Socket 높이 (M_h)", self.latch_vars, self.latch_sliders, self.latch_spinboxes, "d_h", 8.0, 12.0)
        self.create_slider(self.latch_input_group, "Dial 값 (Dial)", self.latch_vars, self.latch_sliders, self.latch_spinboxes, "dial_val", 8.0, 11.0)
        self.create_slider(self.latch_input_group, "Dial Offset (Offset)", self.latch_vars, self.latch_sliders, self.latch_spinboxes, "dial_offset", 0.0, 2.0)
        ...
        self.latch_res_val1 = tk.Label(self.latch_output_group, text="1) Hard Stop + MPGA Socket (최소 허용치): 0.00 mm", font=("Helvetica", 10))
        ...
        self.latch_res_compression = tk.Label(self.latch_output_group, text="Rubber Socket 압축량 : 0.00 mm", font=("Helvetica", 10))
        ...
        self.heatsink_spinboxes = {}
        self.heatsink_sliders = {}
        self.create_slider(self.heatsink_input_group, "Rubber 최대 높이 (S_max)", self.heatsink_vars, self.heatsink_sliders, self.heatsink_spinboxes, "s_max", 0.1, 1.5)
        self.create_slider(self.heatsink_input_group, "Rubber Hard Stop (S_hard)", self.heatsink_vars, self.heatsink_sliders, self.heatsink_spinboxes, "s_hard", 0.2, 1.3)
        self.create_slider(self.heatsink_input_group, "MPGA Socket 높이 (M_h)", self.heatsink_vars, self.heatsink_sliders, self.heatsink_spinboxes, "d_h", 8.0, 12.0)
        self.create_slider(self.heatsink_input_group, "Socket Guide 높이 (G_h)", self.heatsink_vars, self.heatsink_sliders, self.heatsink_spinboxes, "g_h", 3.0, 6.0)
        self.create_slider(self.heatsink_input_group, "Insert 높이 (I_h)", self.heatsink_vars, self.heatsink_sliders, self.heatsink_spinboxes, "i_h", 3.0, 6.0)
        self.create_slider(self.heatsink_input_group, "Heatsink 홈 깊이", self.heatsink_vars, self.heatsink_sliders, self.heatsink_spinboxes, "h_g", 0.0, 3.0)
        
        # 2-3) 연산 결과 구역
        self.heatsink_output_group = ttk.LabelFrame(left_panel, text=" 연산 결과 (Outputs) ", padding=10)
        self.heatsink_res_val1 = tk.Label(self.heatsink_output_group, text="1) Hard Stop + MPGA Socket(최소 허용치): 0.00 mm", font=("Helvetica", 10))
        ...
        self.heatsink_res_compression = tk.Label(self.heatsink_output_group, text="Rubber Socket 압축 길이: 0.00 mm", font=("Helvetica", 10))
    ...
    def create_slider(self, parent, text, var_dict, slider_dict, spinbox_dict, var_name, from_, to_):
        ...
        # Dial 값(dial_val)은 스핀박스 증감 단위를 0.05로, 그 외는 0.01로 지정
        inc = 0.05 if var_name == "dial_val" else 0.01
        
        # 1. Spinbox (직접 입력 및 단위 조절 가능)
        # width를 7로 조절하고, justify="right"로 숫자가 오른쪽 정렬되도록 하여 깔끔하게 배치합니다.
        # textvariable 바인딩 시 수동 갱신과의 충돌로 인한 텍스트 실종 방지를 위해 제외합니다.
        sp = ttk.Spinbox(
            frame, 
            from_=from_, 
            to=to_, 
        ...
        sp.pack(side="right", padx=(5, 0))
        
        # 초기값 입력
        sp.insert(0, f"{var.get():.2f}")
        ...
        scale = ttk.Scale(
            ...
            to=to_, 
            variable=var, 
            command=lambda e, vn=var_name, f=from_, t=to_: self.on_scale_changed(vn, f, t)
        )
        scale.pack(side="right", fill="x", expand=True, padx=5)
        
        # Spinbox 이벤트 바인딩 (엔터 입력 및 포커스 아웃 시 유효성 검사 및 정밀화)
        sp.bind("<Return>", lambda e, vn=var_name, f=from_, t=to_: self.on_entry_changed(vn, f, t))
        sp.bind("<FocusOut>", lambda e, vn=var_name, f=from_, t=to_: self.on_entry_changed(vn, f, t))
        # Spinbox 증감 화살표를 누를 때 즉각 반영되도록 command 콜백 설정
        sp.config(command=lambda vn=var_name, f=from_, t=to_: self.on_entry_changed(vn, f, t))
        
        slider_dict[var_name] = scale
        spinbox_dict[var_name] = sp

    def on_scale_changed(self, var_name, from_val, to_val):
        """슬라이더 조작 시 스냅 처리 및 시뮬레이션 업데이트 (Dial은 0.05 단위, 그 외는 0.01 단위)"""
        vars_dict = self.latch_vars if self.current_mode == "Latch" else self.heatsink_vars
        ...
        # 0.05 혹은 0.01 단위로 스냅 및 범위 제한
        if var_name == "dial_val":
            stepped_val = round(val * 20.0) / 20.0
        else:
            stepped_val = round(val * 100.0) / 100.0
            
        if stepped_val < from_val:
            stepped_val = from_val
        elif stepped_val > to_val:
            stepped_val = to_val
            
        var.set(stepped_val)
        self.update_simulation()

    def on_entry_changed(self, var_name, from_val, to_val):
        """Spinbox 수치 입력 완료 및 증감 시 유효성 검사 및 0.01 단위 정규화"""
        vars_dict = self.latch_vars if self.current_mode == "Latch" else self.heatsink_vars
        spinboxes_dict = self.latch_spinboxes if self.current_mode == "Latch" else self.heatsink_spinboxes
        var = vars_dict[var_name]
        sp = spinboxes_dict[var_name]
        
        raw_text = sp.get()
        try:
            val = float(raw_text)
        except ValueError:
            # 숫자가 아닐 시 기존 내부의 유효한 값으로 복구
            try:
                val = var.get()
            except tk.TclError:
                val = from_val
    ...
    def on_latch_recipe_changed(self, event):
        ...
        if recipe == "HBM3E":
            ...
            self.latch_spinboxes["s_max"].config(state="disabled")
            self.latch_spinboxes["s_hard"].config(state="disabled")
        elif recipe == "HBM4":
            ...
            self.latch_spinboxes["s_max"].config(state="normal")
            self.latch_spinboxes["s_hard"].config(state="normal")
    ...
    def on_latch_offset_changed(self, event):
        ...
        if offset_recipe == "HBM3E Latch":
            ...
            self.latch_spinboxes["dial_offset"].config(state="disabled")
            ...
            self.latch_spinboxes["dial_offset"].config(state="normal")
    ...
    def on_heatsink_recipe_changed(self, event):
        ...
        if recipe == "HBM3E":
            ...
            self.heatsink_spinboxes["s_max"].config(state="disabled")
            self.heatsink_spinboxes["s_hard"].config(state="disabled")
        elif recipe == "HBM4":
            ...
            self.heatsink_spinboxes["s_max"].config(state="normal")
            self.heatsink_spinboxes["s_hard"].config(state="normal")
            
        self.update_simulation()

    def update_simulation(self):
        # 1. 스냅 처리 (0.01 단위 스냅) 및 물리량 가져오기
        if self.current_mode == "Latch":
            ...
            # 스핀박스 텍스트 포맷팅 갱신 (포커스가 해당 위젯에 없을 때만 동기화하여 타이핑 방해 방지)
            for key, sp in self.latch_spinboxes.items():
                if self.root.focus_get() != sp:
                    try:
                        current_state = sp.cget("state")
                        sp.config(state="normal")
                        sp.delete(0, "end")
                        sp.insert(0, f"{self.latch_vars[key].get():.2f}")
                        sp.config(state=current_state)
                    except tk.TclError:
                        pass
            ...
            self.latch_res_val1.config(text=f"1) Hard Stop + MPGA Socket(최소 허용치): {val1:.2f} mm")
            ...
            if space < val1 - 1e-5:
                ...
                self.latch_res_compression.config(text=f"Rubber Socket 압축 길이: {comp_len:.2f} mm (Hard Stop 도달)")
                self.latch_res_gap.config(text=f"MPGA Socket 간섭량 (Crushed): {short_len:.2f} mm", fg="#D32F2F")
                heatsink_val = space
                socket_val = s_hard
                device_crushed = True
                ...
                self.latch_res_status.config(text=f"결과: {status}", fg=color)
                self.latch_res_compression.config(text=f"Rubber Socket 압축 길이: {comp_len:.2f} mm (미압축)")
                ...
                self.latch_res_status.config(text=f"결과: {status}", fg=color)
                self.latch_res_compression.config(text=f"Rubber Socket 압축 길이: {comp_len:.2f} mm")
            ...
            # 스핀박스 텍스트 포맷팅 갱신 (포커스가 해당 위젯에 없을 때만 동기화하여 타이핑 방해 방지)
            for key, sp in self.heatsink_spinboxes.items():
                if self.root.focus_get() != sp:
                    try:
                        current_state = sp.cget("state")
                        sp.config(state="normal")
                        sp.delete(0, "end")
                        sp.insert(0, f"{self.heatsink_vars[key].get():.2f}")
                        sp.config(state=current_state)
                    except tk.TclError:
                        pass
            ...
            self.heatsink_res_val1.config(text=f"1) Hard Stop + MPGA Socket(최소 허용치): {val1:.2f} mm")
            ...
            if val2 < val1 - 1e-5:
                ...
                self.heatsink_res_compression.config(text=f"Rubber Socket 압축 길이: {comp_len:.2f} mm (Hard Stop 도달)")
                self.heatsink_res_gap.config(text=f"MPGA Socket 간섭량 (Crushed): {short_len:.2f} mm", fg="#D32F2F")
                heatsink_val = val2
                socket_val = s_hard
                device_crushed = True
                ...
                self.heatsink_res_status.config(text=f"결과: {status}", fg=color)
                self.heatsink_res_compression.config(text=f"Rubber Socket 압축 길이: {comp_len:.2f} mm (미압축)")
                ...
                self.heatsink_res_status.config(text=f"결과: {status}", fg=color)
                self.heatsink_res_compression.config(text=f"Rubber Socket 압축 길이: {comp_len:.2f} mm")
        ...
        if self.current_mode == "Latch":
            ...
            if device_crushed:
                ...
                self.canvas.create_text(cx, (p_dev_top + p_dev_bottom)/2, text="MPGA Socket CRUSHED", fill="white", font=("Helvetica", 10, "bold"))
            else:
                ...
                self.canvas.create_text(cx, (p_dev_top + p_dev_bottom)/2, text="MPGA Socket", fill="white", font=("Helvetica", 10, "bold"))
            ...
            self.canvas.create_text(cx - 350, limit_y - 10, text=f"Limit (Hard Stop+MPGA): {val1:.2f} mm", fill="#D32F2F", font=("Helvetica", 9, "bold"), anchor="w")
            ...
            items = [
                ...
                ("#FFCA28", "Rubber Socket (러버 소켓 핀)"),
                ("#2196F3", "MPGA Socket (MPGA Socket)"),
            ...
            # --- [Yellow: Rubber Socket] ---
            soc_w = 180
            p_soc_top = base_y - socket_val * SCALE
            self.canvas.create_rectangle(cx - soc_w/2, p_soc_top, cx + soc_w/2, base_y, fill="#FFCA28", outline="#FF8F00", width=2)
            self.canvas.create_text(cx, (p_soc_top + base_y)/2, text="Rubber Socket", fill="#5D4037", font=("Helvetica", 10, "bold"))
            ...
            self.canvas.create_text(cx - 320, val1_y - 10, text="1) Hard Stop + MPGA Socket", fill="red", font=("Helvetica", 9, "bold"), anchor="w")
            ...
            items = [
                ("#FFCA28", "Yellow: Rubber Socket (PCB에 고정)"),
                ("#424242", "Black: Socket Guide (Socket 주변)"),
                ("#42A5F5", "Blue: MPGA Socket (Socket 위 배치)"),
            ...
            self.canvas.create_text(
                legend_x + 15, curr_y,
                text="조건:\n1) <= 2) < (Rubber 최대 높이 + MPGA Socket 높이)",
